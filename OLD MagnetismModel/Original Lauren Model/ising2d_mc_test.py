...
## Change: L=6, n_measSweeps = N_spins, neighbours,plot interpolate, plot animate, plot time
...
for T in T_list:
  ...
  for i in range(n_measSweeps):
    ...
    if animate and i == n_measSweeps-1:
      #Display the current spin configuration:
      plt.clf()
      plt.imshow( spins.reshape((L,L)), cmap=bw_cmap, norm=colors.BoundaryNorm([-1,0,1], 
