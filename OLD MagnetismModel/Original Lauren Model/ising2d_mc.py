...
n_eqSweeps = L**(d-1)   #number of equilibration sweeps
n_measSweeps = L**(d)  #number of measurement sweeps
...
neighbours = np.zeros((N_spins,2*d),dtype=np.int)
for i in range(N_spins):
...
def sweep():
  ...
  for i in range(N_spins):
    ...
    ##E_final = getEnergy()    
    deltaE = (sum(-2*J*(spins[site]*spins[neighbours[site,i]]) for i in range(2**d)))   
...
for T in T_list:
  ...
  for i in range(n_measSweeps):
    ...
    if animate and (i%n_measSweeps-1==0):
