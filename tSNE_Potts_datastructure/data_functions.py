...
class Data_Process(object):
			...
			for ifig in set([f.number for f in fig_i.values()]):
				...
				if data_params.get('data_file'):
					file = os.path.join(directory,
									data_params.get('data_file') + '_' + (
									fig_k + label))
				else:
					file = os.path.join(directory,fig_k + label)
	...
	def importer(self,data_params = 
		...
		def import_func(file,directory,format,key,dtype=None,**kwargs):		
			...
			if not os.path.getsize(os.path.join(directory,
									formatter(file,format)))> 0:
				return None			
			...
			if format == 'values':
				...
				data = np.load(os.path.join(directory,formatter(file,format)))
			...
			for f in np.atleast_1d(data_files):
				...
				if '*' in f:
					ftemp = [os.path.basename(x) 
								for x in glob.glob(os.path.join(directory,f))]
				else:
	...
	def exporter(self,data,
	   ...
	   # Check if Data is dict type
		...
		for k,v in data.items():
			...
			while os.path.isfile(os.path.join(directory,file_k+
								 file_end+ '.' +format[k])):
				file_end = '_%d'%i
				i+=1
			
			file_path = os.path.join(directory,file_k)
